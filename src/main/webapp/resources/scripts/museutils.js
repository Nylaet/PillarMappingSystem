...
Muse.Utils.getPageStyleSheets = function () {
    for (var a = [], b = 0; b < document.styleSheets.length; ++b) {
        var c = document.styleSheets[b], d = c.ownerNode ? c.ownerNode : c.owningElement;
...
Muse.Utils.allStyleSheetRulesFromOneSheet = function (a, b) {
    ...
    for (var f = [], j = $(window), h = function (a) {
        ...
        if (a.media.mediaText && 0 <= a.media.mediaText.indexOf(b.replace(/\s/g, "")))
            return!0;
        if (a.media.mediaText && (a = a.media.mediaText.replace(/\sand\s/g, "__and__").replace(/\s/g, "").split("__and__"), b = b.replace(/\sand\s/g, "__and__").replace(/\s/g,
                "").split("__and__"), a && a.sort && b && b.sort)) {
...
Muse.Utils.getCanvasDirection = function (a, b) {
    ...
    return{dir: c >= 0 && c <= 45 || c >= 135 && c <= 225 || c >= 315 && c < 360 ? b : b === "horizontal" ? "vertical" : "horizontal", reverse: b === "horizontal" ? c >= 135 && c <= 315 : c >= 45 && c <= 225}
};
...
Muse.Utils.processHyperlink = function (a) {
    ...
    if (!d || d == "_self") {
        ...
        if (f && d > g) {
            var a = c.data("scrollWrapper"), j = b.substring(d), d = Muse.Utils.getAnchorWithDestination(j).offset(), b = f[1], h = a || window, g = document.documentElement || document.body, f = (a ? a.scrollHeight() : g.scrollHeight) - c.height(), c = (a ? a.scrollWidth() : g.scrollWidth) - c.width(), i = Math.min(f, d.top + (a && !a.isStandard() ? a.scrollTop() :
                    0)), k = Math.min(c, d.left + (a && !a.isStandard() ? a.scrollLeft() : 0)), c = function () {
                h.scrollTo(k, i);
                try {
            ...
            if (window.scrollTo || void 0 !== a) {
                var a = a || $(document), m = a.scrollLeft(), o = a.scrollTop(), r = m, p = o;
                $({scrollDistance: 0}).animate({scrollDistance: 1}, {duration: 1E3, easing: b, step: function (a) {
                        a != 0 && (p = a * (i - o), r = a * (k - m), h.scrollTo(m + r, o + p))
                    }, complete: c})
            } else
                $("html,body").animate({scrollTop: i,
                    scrollLeft: k}, 1E3, b, c);
            return!1
        }
    }
...
Muse.Utils.enableAnchorLinksActiveState = function () {
    var a = !1, b = [], c = $(window), d = Muse.Utils.getPageStyleSheets(), g = function (a) {
        ...
        f = !0
    }, h = !1, i = c.data("scrollWrapper"), k = i || c, l = null, m = function () {
        h = !1;
        if (!f) {
            var c = $("#page");
        ...
        for (d = Math.min(d + 2, b.length); o < d; o++)
            if (q = b[o], m = q.$elem.offset().left + (i && !i.isStandard() ? i.scrollLeft() : 0), y = q.$elem.offset().top + (i && !i.isStandard() ? i.scrollTop() : 0), q.from != (a ? m :
                    y)) {
                j(l);
                break
            }
        ...
        for (d = b.length; o < d; o++) {
            ...
            for (var C = 0; C < q.length; C++)
                y = g(q[C]), m && !q[C].hasClass(y) ? q[C].addClass(y) : !m && q[C].hasClass(y) && q[C].removeClass(y)
        }
    }, o = function () {
        h || (h = !0, Muse.Utils.requestAnimationFrame(m))
    };
...
Muse.Utils.saveHyperlinkInfo = function (a, b, c, d, g) {
    var f = a.attr("href"), j = Muse.Utils.pathOnly(f), h = -1, i = a.attr("target"), k = window.location.href.replace(/#.*$/i, "");
    if (f && -1 != f.indexOf("#") && !(i && i != "_self") && !(0 <= j.indexOf("/")) && (k.charAt(k.length - 1) == "/" && (k += "index.html"), -1 != k.indexOf("/" + j, k.length - j.length - 1))) {
        var j = $(window).data("scrollWrapper"), l = f.substring(f.lastIndexOf("#")), a = a.parent('[class~="sbg"]').length > 0 || a.hasClass("block") ? a.parent() : a, m = "#" + a.attr("id"), b = "." + b;
...
Muse.Utils.needPIE = function (a) {
    if (Muse.Utils.havePIE)
        ...
        Muse.Utils.pieLoading = !0, a = "scripts/pie.js", a[0] == "/" && (a = location.pathname.indexOf(".html") != -1 ? location.pathname.substring(0, location.pathname.lastIndexOf("/")) + a : location.pathname + a, a = a.replace(/\/+/g, "/")), $.ajax({url: a, dataType: "script", complete: function () {
...
Muse.Utils.maintainFluidSVGsAspectRatio = function () {
    ...
    if (0 != a.length) {
        var b = function () {
            a.each(function () {
                var a = $(this), b = a.data("image-width"), c = a.data("image-height");
...
Muse.Utils.resizeHeight = function (a) {
    var b = function (a, b) {
        var g = a.parent().hasClass("sbg") ? a.parent() : a, f = $(a.children()[0]);
...
Muse.Utils.fixTransformRotations = function () {
    Muse.Browser.Features.checkCSSFeature("transform") || $("*[data-mu-ie-matrix]").each(function () {
        ...
        a.css({filter: function (b, c) {
                if (c)
                    return c + " " + a.data("mu-ie-matrix");
                return a.data("mu-ie-matrix")
            }, "margin-bottom": "-=" + d}).removeClass("shadow");
        b.css({"margin-bottom": "-=" + (b.innerHeight() - g), "margin-right": "-=" + (b.innerWidth() -
                    f)});
...
Muse.Utils.fullPage = function (a) {
    ...
    if (0 == $(a).closest(".breakpoint").length)
        Muse.Assert.assert(0 == $(".breakpoint").length, "Page is outside a breakpoint node."), b.init($(a));
    else {
        var c = function (c) {
...
Muse.Utils.initWidget = function (a, b, c) {
    if (0 == $(".breakpoint").length)
        ...
        for (var d = 0; d < b.length; d++) {
            ...
            Muse.Utils._initWidgetQueue[g].push({id: a, fn: c})
        }
            ...
            var f = function (a, b, c, d) {
                ...
                if (Muse.Utils._initWidgetQueue[a]) {
                    for (; Muse.Utils._initWidgetQueue[a].length; )
                        if (b = Muse.Utils._initWidgetQueue[a].shift(), b = b.fn($(b.id, c)))
                            b.$bp = c, b.breakpoint = d;
                    ...
                    if (c)
                        $("body").off("muse_bp_activate", f), Muse.Utils._hasBPListener = !1
                }
            };
        ...
        if (!a.is("object") || !(jQuery.browser.msie && jQuery.browser.version < 9)) {
            if (a.is("iframe")) {
                ...
                if (0 <= g.indexOf("vimeo.com")) {
                    ...
                    c.push({$node: a, playFn: function (a) {
...
Muse.Utils.attachIframesAndObjectsToResumeMedia = function (a) {
    ...
    $("video", a).each(function () {
        if (this.play &&
                this.getAttribute("autoplay") && this.paused)
...
Muse.Utils.VimeoVideoHelper = function (a) {
    var b = [], c = function (a, b) {
        ...
        if (c) {
            ...
            for (l = 0; l < b.length; )
                f = b[l], f.source == a.source && f.method == c.method ? (f.callbackFn(c.value), b.splice(l, 1)) : l++
        }
    };
...
Muse.Browser.Features.Touch = function () {
    if (navigator.maxTouchPoints > 0 || window.matchMedia && window.matchMedia("(-moz-touch-enabled)").matches)
        return{Start: "pointerDown", End: "pointerUp", Move: "pointerMove", Listener: function (a) {
                return function (b) {
        ...
        for (var a = 0, b = Muse.Browser.domPrefixes.length; a < b; a++) {
            ...
            if (c + "MaxTouchPoints"in navigator && navigator[c + "MaxTouchPoints"])
                return c =
                        c.toUpperCase(), {Start: c + "PointerDown", End: c + "PointerUp", Move: c + "PointerMove", Listener: function (a) {
                        return function (b) {
    ...
    try {
        return document.createEvent("TouchEvent"), {Start: "touchstart", End: "touchend", Move: "touchmove", Listener: function (a) {
                return a
            }}
    } catch (d) {
    }
...
Muse.Browser.Bugs.ScrollWidthHeightIncludesBorder = function () {
    var a = !1, b = $("<div>").css({border: "1px solid #000000;", width: 100, height: 100, position: "absolute", top: -99999, left: -99999, padding: 0, margin: 0, overflow: "auto"}).appendTo(document.body)[0];
            ...
            if (this.initialized) {
                ...
                if (a != this.spacerHeight) {
                    ...
                    if (a < this.spacerHeight && c == this.$page.outerHeight(!0))
                        a = this.spacerHeight, this.$spacer.css("height", a);
                    this.pageResizeWatchEnabled = !0
                }
        ...
        if (-1 != a) {
            ...
            for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
                ...
                if ("generator" == d.getAttribute("name")) {
                    "2015.1.0.342" != d.getAttribute("content") && Muse.assets.outOfDate.push("museutils.js");
                    break
                }
            }
        }
